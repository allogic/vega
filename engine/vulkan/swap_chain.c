...
uint32_t g_vulkan_swap_chain_min_image_count = 0;
...
void vulkan_swap_chain_alloc(void)
{
	...
	g_vulkan_swap_chain_min_image_count = g_vulkan_surface_capabilities.minImageCount + 1;
	...
	swap_chain_create_info.minImageCount = g_vulkan_swap_chain_min_image_count;
	swap_chain_create_info.imageFormat = g_vulkan_surface_prefered_format.format;
	swap_chain_create_info.imageColorSpace = g_vulkan_surface_prefered_format.colorSpace;
	swap_chain_create_info.imageExtent.width = g_vulkan_surface_width;
	swap_chain_create_info.imageExtent.height = g_vulkan_surface_height;
	swap_chain_create_info.imageArrayLayers = 1;
	swap_chain_create_info.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
	swap_chain_create_info.preTransform = g_vulkan_surface_capabilities.currentTransform;
	swap_chain_create_info.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;
	swap_chain_create_info.presentMode = g_vulkan_surface_prefered_present_mode;
	swap_chain_create_info.clipped = 1;
	swap_chain_create_info.oldSwapchain = 0;

	int32_t queue_families[2] = { g_vulkan_device_graphic_queue_index, g_vulkan_device_present_queue_index };

	if (g_vulkan_device_graphic_queue_index == g_vulkan_device_present_queue_index)
	{
	...
	else
	{
		...
		swap_chain_create_info.queueFamilyIndexCount = VEGA_ARRAY_COUNT(queue_families);
	}

	vkCreateSwapchainKHR(g_vulkan_device, &swap_chain_create_info, 0, &g_vulkan_swap_chain);
...
void vulkan_swap_chain_free(void)
{
	...
	vkDestroySwapchainKHR(g_vulkan_device, g_vulkan_swap_chain, 0);
...
void vulkan_swap_chain_images_alloc(void)
{
	...
	vkGetSwapchainImagesKHR(g_vulkan_device, g_vulkan_swap_chain, &g_vulkan_swap_chain_image_count, 0);

	std_vector_resize(&s_vulkan_swap_chain_images, g_vulkan_swap_chain_image_count);

	vkGetSwapchainImagesKHR(g_vulkan_device, g_vulkan_swap_chain, &g_vulkan_swap_chain_image_count, std_vector_buffer(&s_vulkan_swap_chain_images));

	g_vulkan_swap_chain_depth_format = vulkan_image_find_depth_format();
	g_vulkan_swap_chain_depth_image = vulkan_image_2d_depth_alloc(g_vulkan_surface_width, g_vulkan_surface_height, g_vulkan_swap_chain_depth_format);
...
void vulkan_swap_chain_image_views_alloc(void)
{
	...
	while (swap_chain_image_index < g_vulkan_swap_chain_image_count)
	{
		...
		*image_view = vulkan_image_view_alloc(image, VK_IMAGE_VIEW_TYPE_2D, g_vulkan_surface_prefered_format.format, VK_IMAGE_ASPECT_COLOR_BIT);
