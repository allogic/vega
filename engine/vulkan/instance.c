...
VkPhysicalDevice g_vulkan_physical_device = 0;
VkPhysicalDeviceProperties g_vulkan_physical_device_properties = { 0 };
VkPhysicalDeviceFeatures g_vulkan_physical_device_features = { 0 };

int32_t g_vulkan_graphic_and_compute_queue_index = -1;
int32_t g_vulkan_present_queue_index = -1;

VkDevice g_vulkan_device = 0;

VkQueue g_vulkan_graphic_queue = 0;
VkQueue g_vulkan_compute_queue = 0;
VkQueue g_vulkan_present_queue = 0;

VkSurfaceFormatKHR g_vulkan_prefered_surface_format = { 0 };
VkPresentModeKHR g_vulkan_prefered_present_mode = { 0 };

VkCommandPool g_vulkan_command_pool = 0;

#ifdef VEGA_DEBUG
static PFN_vkCreateDebugUtilsMessengerEXT s_vulkan_create_debug_utils_messenger_ext = 0;
static PFN_vkDestroyDebugUtilsMessengerEXT s_destroy_debug_utils_messenger_ext = 0;

static VkDebugUtilsMessengerEXT s_vulkan_debug_messenger = 0;
#endif VEGA_DEBUG

static char const* s_vulkan_layer_extensions[] =
{
	"VK_KHR_surface",
...
#ifdef VEGA_DEBUG
static char const* s_vulkan_validation_layers[] =
{
...
void vulkan_instance_alloc(void)
{
	...
	instance_create_info.enabledExtensionCount = ARRAY_COUNT(s_vulkan_layer_extensions);
	instance_create_info.ppEnabledExtensionNames = s_vulkan_layer_extensions;
	...
	instance_create_info.enabledLayerCount = ARRAY_COUNT(s_vulkan_validation_layers);
	instance_create_info.ppEnabledLayerNames = s_vulkan_validation_layers;
#endif // VEGA_DEBUG

	vkCreateInstance(&instance_create_info, 0, &g_vulkan_instance);

#ifdef VEGA_DEBUG
	s_vulkan_create_debug_utils_messenger_ext = (PFN_vkCreateDebugUtilsMessengerEXT)vkGetInstanceProcAddr(g_vulkan_instance, "vkCreateDebugUtilsMessengerEXT");
	s_destroy_debug_utils_messenger_ext = (PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(g_vulkan_instance, "vkDestroyDebugUtilsMessengerEXT");

	s_vulkan_create_debug_utils_messenger_ext(g_vulkan_instance, &debug_create_info, 0, &s_vulkan_debug_messenger);
#endif // VEGA_DEBUG
...
#ifdef VEGA_DEBUG
	s_destroy_debug_utils_messenger_ext(g_vulkan_instance, s_vulkan_debug_messenger, 0);
#endif
...
void vulkan_instance_device_alloc(void)
{
...

	{
		...
		device_queue_create_info.queueFamilyIndex = g_vulkan_graphic_and_compute_queue_index;
...

	{
		...
		device_queue_create_info.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
		device_queue_create_info.queueFamilyIndex = g_vulkan_present_queue_index;
		device_queue_create_info.queueCount = 1;
		device_queue_create_info.pQueuePriorities = &queue_priority;
	...
	vkGetPhysicalDeviceFeatures2(g_vulkan_physical_device, &physical_device_extended_features);
	vkGetPhysicalDeviceSurfaceCapabilitiesKHR(g_vulkan_physical_device, g_vulkan_surface, &g_vulkan_surface_capabilities);
	...
	device_create_info.enabledExtensionCount = ARRAY_COUNT(s_vulkan_device_extensions);

#ifdef VEGA_DEBUG
	device_create_info.ppEnabledLayerNames = s_vulkan_validation_layers;
	device_create_info.enabledLayerCount = ARRAY_COUNT(s_vulkan_validation_layers);
#endif

	vkCreateDevice(g_vulkan_physical_device, &device_create_info, 0, &g_vulkan_device);

	vkGetDeviceQueue(g_vulkan_device, g_vulkan_graphic_and_compute_queue_index, 0, &g_vulkan_graphic_queue);
	vkGetDeviceQueue(g_vulkan_device, g_vulkan_graphic_and_compute_queue_index, 0, &g_vulkan_compute_queue);
	vkGetDeviceQueue(g_vulkan_device, g_vulkan_present_queue_index, 0, &g_vulkan_present_queue);
...
void vulkan_instance_command_pool_alloc(void)
{
	...
	command_pool_create_info.queueFamilyIndex = g_vulkan_graphic_and_compute_queue_index;
...
void vulkan_instance_find_physical_device(void)
{
	...
	while (physical_device_index < physical_device_count)
	{
		...
		vkGetPhysicalDeviceProperties(physical_device, &g_vulkan_physical_device_properties);
		vkGetPhysicalDeviceFeatures(physical_device, &g_vulkan_physical_device_features);

		if (g_vulkan_physical_device_properties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
		{
			if (g_vulkan_physical_device_features.geometryShader && g_vulkan_physical_device_features.samplerAnisotropy)
			{
				g_vulkan_physical_device = physical_device;
...
void vulkan_instance_find_physical_device_queue_families(void)
{
	...
	vkGetPhysicalDeviceQueueFamilyProperties(g_vulkan_physical_device, &queue_family_property_count, 0);
	...
	vkGetPhysicalDeviceQueueFamilyProperties(g_vulkan_physical_device, &queue_family_property_count, std_vector_buffer(&queue_family_properties));
	...
	while (physical_device_queue_family_property_index < queue_family_property_count)
	{
		...
		vkGetPhysicalDeviceSurfaceSupportKHR(g_vulkan_physical_device, (uint32_t)physical_device_queue_family_property_index, g_vulkan_surface, &present_support);

		if (graphic_support && compute_support && (g_vulkan_graphic_and_compute_queue_index == -1))
		{
			g_vulkan_graphic_and_compute_queue_index = (uint32_t)physical_device_queue_family_property_index;
		}
		else if (present_support && (g_vulkan_present_queue_index == -1))
		{
			g_vulkan_present_queue_index = (uint32_t)physical_device_queue_family_property_index;
		}

		if ((g_vulkan_graphic_and_compute_queue_index != -1) && (g_vulkan_present_queue_index != -1))
		{
			break;
		}
	...
	printf("\tGraphicAndComputeQueueIndex %d\n", g_vulkan_graphic_and_compute_queue_index);
	printf("\tPresentQueueIndex %d\n", g_vulkan_present_queue_index);
	printf("\n");
...
void vulkan_instance_check_physical_device_extensions(void)
{
	...
	vkEnumerateDeviceExtensionProperties(g_vulkan_physical_device, 0, &available_device_extension_count, 0);
	...
	vkEnumerateDeviceExtensionProperties(g_vulkan_physical_device, 0, &available_device_extension_count, std_vector_buffer(&available_device_extensions));

	printf("Device Extensions\n");

	uint64_t device_extension_index = 0;
	uint64_t device_extension_count = ARRAY_COUNT(s_vulkan_device_extensions);
...
void vulkan_instance_check_surface_capabilities(void)
{
	...
	vkGetPhysicalDeviceSurfaceFormatsKHR(g_vulkan_physical_device, g_vulkan_surface, &surface_format_count, 0);
	vkGetPhysicalDeviceSurfacePresentModesKHR(g_vulkan_physical_device, g_vulkan_surface, &present_mode_count, 0);
	...
	vkGetPhysicalDeviceSurfaceFormatsKHR(g_vulkan_physical_device, g_vulkan_surface, &surface_format_count, std_vector_buffer(&surface_formats));
	vkGetPhysicalDeviceSurfacePresentModesKHR(g_vulkan_physical_device, g_vulkan_surface, &present_mode_count, std_vector_buffer(&present_modes));
	...
	while (surface_format_index < surface_format_count)
	{
		...
		{
			g_vulkan_prefered_surface_format = surface_format;
	...
	while (present_mode_index < present_mode_count)
	{
		...
		{
			g_vulkan_prefered_present_mode = present_mode;
